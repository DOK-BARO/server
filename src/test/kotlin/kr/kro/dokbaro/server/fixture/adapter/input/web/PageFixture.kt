...
import kr.kro.dokbaro.server.configuration.docs.means
...
inline fun <reified T : Enum<T>> pageQueryParameters(): List<Parameter> =
	listOf(
		"page" means "결과 페이지 번호. 1부터 시작.",
		"size" means "페이지당 결과 수.",
		"sort" means "정렬 기준. [${enumValues<T>().joinToString(", ") { it.name }}]",
		"direction" means "정렬 방향. 가능한 값은 'ASC' 또는 'DESC'",
	)

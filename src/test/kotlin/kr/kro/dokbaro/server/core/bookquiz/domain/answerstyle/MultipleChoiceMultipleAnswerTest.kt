...
class MultipleChoiceMultipleAnswerTest :
	StringSpec({

		"답변의 타입을 확인한다" {
			GradeSheetFactory
				.create(QuizType.MULTIPLE_CHOICE_MULTIPLE_ANSWER, AnswerSheet(listOf("1", "2", "3")))
				.getType() shouldBe
		...
		"일치 여부를 확인한다" {
			...
			answer.isCorrect(AnswerSheet(listOf("4", "5"))) shouldBe false
			answer.isCorrect(AnswerSheet(listOf("1", "6"))) shouldBe false
		...
		"답변 제출 시 숫자 형태가 아닌 값을 제출 시 예외를 반환한다" {
			shouldThrow<IllegalSubmitSheetFormatException> {
				MultipleChoiceMultipleAnswer
					.from(AnswerSheet(listOf("1", "2")))
					.isCorrect(AnswerSheet(listOf("B", "C")))
			}
		}
		...
		"문제 생성 시 객관식 답이 하나면 예외를 반환한다" {
			shouldThrow<IllegalRegisterSheetFormatException> {
				MultipleChoiceMultipleAnswer.from(AnswerSheet(listOf("1")))
			}
		}

		"담변 제출 시 객관식 답이 하나면 예외를 반환한다" {
			shouldThrow<IllegalSubmitSheetFormatException> {
				MultipleChoiceMultipleAnswer.from(AnswerSheet(listOf("1", "2"))).isCorrect(AnswerSheet(listOf("1")))
			}
		}
	})
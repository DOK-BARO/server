...
class StudyGroupQueryControllerTest : RestDocsTest() {
	...
	@MockkBean
	lateinit var findStudyGroupDetailByInviteCodeUseCase: FindStudyGroupDetailByInviteCodeUseCase
	...
	init {
		...
		"invite code를 통한 스터디 조회를 수행한다" {
			every { findStudyGroupDetailByInviteCodeUseCase.findStudyGroupDetailByInviteCode(any()) } returns
				StudyGroupDetail(
					name = "AI 연구 스터디",
					introduction = "인공지능과 머신러닝에 대해 함께 공부하는 스터디입니다.",
					profileImageUrl = "https://example.com/group-profile.jpg",
			...
			performGet(Path("/study-groups/invite-code/{inviteCode}", "ABC123"))
				.andExpect(status().isOk)
				.andDo(
					print(
						"study-group/get-study-group-detail-by-invite-code",
						pathParameters(parameterWithName("inviteCode").description("study group invite code")),
						responseFields(
							fieldWithPath("id").description("스터디 그룹의 고유 ID"),
							fieldWithPath("name").description("스터디 그룹의 이름"),
							fieldWithPath("introduction").description("스터디 그룹 소개").optional(),
